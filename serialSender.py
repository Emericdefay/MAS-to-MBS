...
def start(name="default.mid",arduino_COM="COM3",warningMsg=True):
    ...
    if warningMsg:
        print("\n\tReminder : \n\t  Remember to upload 'serialReaderPrinter.ino' on your arduino board.")
        print("\tThis warning will stay 20 seconds. To make sure that you have the time to check.")
        print("\tYou can disable this reminder : start('MidiFileName.mid',ArduinoPortCOM,->False<-)")
        print("\n\t[",end="")
    ...
    print("\n\tStart to print : {} - Amount of laps : {}.".format(name,len(liste)))
    print("\tAverage time : {} min.\n".format(int((10*len(liste))/60)))
    time.sleep(2)
